...
$(function(){
	...
	$('.select2').each(function(){
		...
		var tags = typeof $t.attr('data-tags') == 'undefined' ? false : true;
	...
	if($('#disabled-form').length > 0) {
		...
		if(typeof $('#modal-form').attr('data-manual') == 'undefined') {
			...
			if($(this).attr('data-id') == '0') {
				...
				if ($.fn.spectrum) {
					$('input.color-picker.spectrum').spectrum("set", '');
				}
				$('.tags').tagsinput();
			} else {
				var getUrl = '';
					...
					success		: function(response) {
						...
						if(typeof response['status'] == 'undefined' || typeof response['message'] == 'undefined') {
							...
							$('#modal-form form :input').each(function(){
									...
									if(typeof res_value != 'undefined' && typeof res_value == 'string' && res_value) {
										...
										if(!is_datetime) {
											if(c_date.length == 3 && c_date[0].length == 4 && c_date[1].length == 2 && c_date[2].length == 2) {
												...
												} else if(typeof response['text_' + $(this).attr('name')] != 'undefined') {
													$(this).html('<option value="'+res_value+'">'+response['text_' + $(this).attr('name')]+'</option>');
												}  else {
								...
								if($.fn.spectrum && $(this).hasClass('color-picker') && $(this).hasClass('spectrum')) {
									$(this).spectrum("set", res_value);
								}
							});
							if(typeof $('#modal-form .modal-title').attr('data-norename') == 'undefined') {	
								$('#modal-form .modal-title').html(mtitle);	
...
$('.btn-act-import').click(function(){
	...
	$('#modal-import select').trigger('change');
...
$(document).ready(function(){
	if(typeof $('#notif-bar').attr('data-count') != 'undefined' && typeof $('#notif-bar').attr('data-toast') != 'undefined') {
		var count_notif = parseInt($('#notif-bar').attr('data-count'));
		if(count_notif > 0 && $('#notif-bar').attr('data-toast') == 'true') {
			$.toast({
				heading: lang.pemberitahuan,
				text: '('+count_notif+') ' + lang.pemberitahuan_baru_belum_dibaca,
				showHideTransition: 'plain',
				icon: 'info',
				position: 'top-right',
				loader: false,
				hideAfter: 5000,
				onClick: function() {
					window.location = base_url + 'home/notification';
				}
			});
		}
	}
	...
	$('select.select2').each(function(){
		if($(this).find('option[data-image]').length > 1) {
			$(this).parent().addClass('select2-image');
		}
	});
});
...
function formatSelect2 (opt) {
	...
	var imageOnly = typeof $(opt.element).attr('data-imageOnly') == 'undefined' ? false : true;
	var size = typeof $(opt.element).attr('data-size') == 'undefined' ? 'width' : $(opt.element).attr('data-size');
	if(size != 'width' && size != 'height') size = "width";
	if(typeof optimage == 'undefined') {
		optimage = '';
	} else {
		var c = optimage.split('/');
		if(c.length == 1) optimage = '';
	}
	if(!optimage){
		return opt.text;
	} else {
		if(imageOnly) {
			var $opt = $(
				'<span class="pt-1 pb-1 d-inline-block"><img src="' + optimage + '" '+size+'="36px" alt="" /></span>'
			);
		} else {
			var $opt = $(
				'<span class="pt-1 pb-1 d-inline-block"><img src="' + optimage + '" '+size+'="36px" class="mr-2" alt="" /> ' + opt.text + '</span>'
			);
		}
		return $opt;
	}
