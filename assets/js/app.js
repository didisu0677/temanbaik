...
$(function(){
	...
	if($('.filter-panel').length > 0) {
		$('.content-header .float-right').first().append('<button class="btn btn-info btn-sm btn-icon-only btn-filter-panel-show" type="button" title="'+lang.filter+'"><i class="fa-sliders-v d-inline"></i> <span class="d-none d-md-inline">'+lang.filter+'</span></button>');
	...
	$('.show-table-border').click(function(){
		if(!$(this).hasClass('current')) {
			$.ajax({
				url : base_url + 'account/show_table_border',
				data : {},
				type : 'post',
				success : function() {
					location.reload();
	...
	$('.menu-display').click(function(){
		if(!$(this).hasClass('current')) {
			$.ajax({
				url : base_url + 'account/change_menu_display',
