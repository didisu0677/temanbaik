...
class Ws {
	...
	public function send($method,$read=false) {
	...
	private function connect() {
		$url_parts 	= parse_url($this->params['host']);
		$scheme		= $url_parts['scheme'];
		$host 		= $url_parts['host'];

		$host_uri = ($scheme === 'wss' ? 'ssl' : 'tcp') . '://' . $host;

		$sock = fsockopen($host_uri, $this->params['port'], $errno, $errstr, 2);
		fwrite($sock, $this->head);
		$headers = fread($sock, 27);
