...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
    ...
    public static function toFormattedString($value = '0', $format = PHPExcel_Style_NumberFormat::FORMAT_GENERAL, $callBack = null)
    {
        ...
        if (!is_numeric($value)) {
            return $value;
        } elseif (strpos($value,'E+') !== false) {

            // KASUS JIKA NILAINYA ERROR E+
            $x_value = explode('E+',$value);
            if(count($x_value) == 2) {
                $pengali    = str_pad(1, $x_value[1], 0, STR_PAD_RIGHT);
                $value      = bcmul($x_value[0], $pengali);
                return $value;
            } else {
                return $value;
            }
