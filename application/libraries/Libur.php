...
class Libur {
	...
	public function get($tahun=false,$start_end=false) {
		if($tahun)
			$this->tahun 	= $tahun;
		...
		foreach($res[1] as $r) {
			...
			if($start_end) { // JIKA MAU DIBUAT DENGAN FORMAT START DATE / END DATE
				if(count($e_str_tanggal) == 1) {
					$result[]		= [
						'tanggal_mulai'		=> date('Y-m-d',strtotime($tanggal)),
						'tanggal_selesai'	=> date('Y-m-d',strtotime($tanggal)),
			...
			} else { // JIKA MAU DIBUAT DENGAN FORMAT LIBUR PER HARI
				if(count($e_str_tanggal) == 1) {
					$result[]		= [
