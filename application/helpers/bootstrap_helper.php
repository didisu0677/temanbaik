    ...
    if(isset($cur_menu->id)) {
        if($cur_menu->level2) {
            ...
            if(isset($menu_l1->id)) {
                ...
                $br[]   = array(
                    ...
                    'title' => lang('_'.$menu_l1->target,$menu_l1->nama)
                );
            }
        }
        if($cur_menu->level3) {
            ...
            if(isset($menu_l2->id)) {
                $br[]   = array(
                    ...
                    'title' => lang('_'.$menu_l2->target,$menu_l2->nama)
                );
            }
        }
        if($cur_menu->level4) {
            ...
            if(isset($menu_l3->id)) {
                $br[]   = array(
                    ...
                    'title' => lang('_'.$menu_l3->target,$menu_l3->nama)
                );
            }
		}
		$show_last_br = true;
		...
		if($show_last_br) {
			$br[]   = array(
				...
				'title' => lang('_'.$cur_menu->target,$cur_menu->nama)
			);
		}
    }
    if($last) {
...
function table_open($class='',$fixed=false,$serverside=false,$table='',$attr='') {
	...
	if(get_cookie('hide_table_border')) {
		$class = str_replace('table-bordered','',$class);
	}
	$CI = get_instance();
	$CI->session->set_userdata('g_table',true);
	$ss 	= $serverside && $table ? ' data-serverside="'.$serverside.'" data-key="'.base64_encode($table).'"' : '';
...
function th($text='',$class='',$attr='') {
	...
	if($CI->session->userdata('g_table')) {
		...
		$cls = $class ? ' class="'.$class.'"' : '';
		$attr = $attr ? ' '.$attr : '';
		if(strpos($attr,'data-table') !== false) {
			$table_name = string_between($attr,'data-table="','"');
			$attr = str_replace('data-table="'.$table_name.'"','data-key="'.base64_encode($table_name).'"',$attr);
		}
...
function form_close() {
	...
	$CI->session->unset_userdata('col_size');
...
function input($tipe='',$label='',$nama='',$validation='',$value='',$attr='',$label_group='') {
	...
	$sz = $CI->session->userdata('col_size') ? $CI->session->userdata('col_size') : 'md';
	if($tipe == 'hidden') {
		...
		if(!$CI->session->userdata('g_inputgroup')) {
			...
			if($c1) {
				$html .= '<label class="col-form-label col-'.$sz.'-'.$c1.$required.$sub.'" for="'.$_id.'">'.$label.'</label>' . PHP_EOL;
			}
			$html .= '<div class="col-'.$sz.'-'.$c2.'">' . PHP_EOL;
		}
		if($tipe == 'icon') {
			if(!$CI->session->userdata('g_inputgroup')) {
			...
			$html .= '<input type="text" name="'.$nama.'" id="'.$_id.'" autocomplete="off" class="form-control '.$tipe.'" data-validation="'.$validation.'" value="'.$value.'"'.$attr.'>' . PHP_EOL;
			if($label_group) {
				if($label_append) {
					$html .= '<div class="input-group-append">' . PHP_EOL;
			...
			if(!$CI->session->userdata('g_inputgroup')) {
				...
				$html .= '<input type="text" name="'.$nama.'" id="'.$_id.'" autocomplete="off" class="form-control tags" data-validation="'.$validation.'" value="'.$value.'"'.$attr.'>' . PHP_EOL;
				if($label_group) {
					if($label_append) {
						$html .= '<div class="input-group-append">' . PHP_EOL;
						$html .= '<span class="input-group-text">'.$label_append.'</span>' . PHP_EOL;
...
function select($label='',$nama='',$validation='',$data=array(),$opt_key="",$opt_val="",$value='',$attr='') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		$html .= '<div class="form-group row">' . PHP_EOL;
		if($c1) {
			$html .= '<label class="col-form-label col-'.$sz.'-'.$c1.$required.$sub.'" for="'.$nama_id.'">'.$label.'</label>' . PHP_EOL;
		}
		$html .= '<div class="col-'.$sz.'-'.$c2.'">' . PHP_EOL;
	}
	...
	foreach($data as $k => $d) {
		if($opt_key == '' || $opt_val == '') {
			...
			if($i == 0 && strpos($validation, 'not_null') === FALSE) {
...
function select2($label='',$nama='',$validation='',$data=array(),$opt_key="",$opt_val="",$value='',$attr='',$opt_attr='') {
	$CI 		= get_instance();
	if($attr) $attr = ' '.$attr;
	if(!is_array($data)) $data = array();
	$nama_id 	= str_replace(['[',']'], '', $nama);
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		foreach($data as $k => $d) {
			if($opt_key == '' || $opt_val == '') {
				...
				$_opt_attr = '';
				$x_opt_attr = explode(' ',$opt_attr);
				foreach($x_opt_attr as $c_attr) {
					$x_c_attr = explode(':',$c_attr);
					if(count($x_c_attr) == 2) {
						$check_dir 	= explode('--',$x_c_attr[1]);
						$dir 		= '';
						if(count($check_dir) == 2) {
							$dir 	= base_url(dir_upload($check_dir[0]));
							$x_c_attr[1] = $check_dir[1];
						}
						if(isset($d[$x_c_attr[1]])) {
							$dir_show = $d[$x_c_attr[1]] ? $dir : '';
							$_opt_attr .= ' '.$x_c_attr[0].'="'.$dir_show.$d[$x_c_attr[1]].'"';
						} else {
							$_opt_attr .= ' '.$x_c_attr[0].'="'.$x_c_attr[1].'"';
						}
					}
				}
				if(is_array($value)) {
					foreach($value as $vl) {
						if($d[$opt_key] == $vl || strtoupper($d[$opt_key]) == $vl) {
							$html .= '<option value="'.$d[$opt_key].'"'.$_opt_attr.' selected>'.$opt_label.'</option>' . PHP_EOL;
						} else {
							$html .= '<option value="'.$d[$opt_key].'"'.$_opt_attr.'>'.$opt_label.'</option>' . PHP_EOL;
						}
					}
				} else {
...
function checkbox($label='',$nama='',$value='',$attr='') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		if(!$CI->session->userdata('g_checkbox')) {
			...
			$html .= '<div class="form-group row">' . PHP_EOL;
			if($c1) {
				$html .= '<label class="col-form-label col-'.$sz.'-'.$c1.$sub.'" for="'.$nama.'">'.$label.'</label>' . PHP_EOL;
			}
			$html .= '<div class="col-'.$sz.'-'.$c2.'">' . PHP_EOL;
		}
...
function checkbox_group($label='') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		$html = '<div class="form-group row">' . PHP_EOL;
		if($c1) {
			$html .= '<label class="col-form-label col-'.$sz.'-'.$c1.'">'.$label.'</label>' . PHP_EOL;
		}
		$html .= '<div class="col-'.$sz.'-'.$c2.'">' . PHP_EOL;
		echo $html;
	}
...
function fileupload($label='',$nama='',$validation='',$attr='') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		...
		$html .= '<div class="form-group row">' . PHP_EOL;
		if($c1) {
			$html .= '<label class="col-form-label col-'.$sz.'-'.$c1.$required.$sub.'" for="'.$nama.'">'.$label.'</label>' . PHP_EOL;
		}
		$html .= '<div class="col-'.$sz.'-'.$c2.'">' . PHP_EOL;
...
function inputgroup_open($label='',$required='') {
	...
	$html .= '<div class="form-group row">' . PHP_EOL;
	if($c1) {
		$html .= '<label class="col-form-label col-'.$sz.'-'.$c1.$sub.' '.$required.'">'.$label.'</label>' . PHP_EOL;
	}
	$html .= '<div class="col-'.$sz.'-'.$c2.'">' . PHP_EOL;
	$html .= '<div class="input-group">' . PHP_EOL;
	echo $html;
...
function form_button($submit='Submit',$reset='Reset') {
	...
	if(!$CI->session->userdata('g_inputgroup')) {
		$html .= '<div class="form-group row">' . PHP_EOL;
		if($c1) {
			$html .= '<div class="col-'.$sz.'-'.$c2.' offset-'.$sz.'-'.$c1.'">' . PHP_EOL;
		} else {
			$html .= '<div class="col-'.$sz.'-'.$c2.'">' . PHP_EOL;
		}
