...
/* function menu($type="all",$segment_f=""){
    ...
    return array(
        ...
        'title'             => isset($cur_menu->id) ? lang('_'.$cur_menu->target,$cur_menu->nama) : '',
...
function menu($type="all",$segment_f=""){
    // tambahan kasus dayu
    $menu_exclusive             = [
        'mitra_baru'            => ['ibeffaro','seemitra']
    ];

    if(isset($menu_exclusive) && count($menu_exclusive)) {
        $usr_menu               = [];
        foreach($menu_exclusive as $k_me => $me) {
            $usr_menu[$k_me]    = [];
            $u = get_data('tbl_user','username',$me)->result();
            foreach($u as $uu) {
                $usr_menu[$k_me][]  = $uu->_key;
            }
        }
    }
    // end tambahan kasus dayu
    ...
    if(user('id_group')) {
        ...
        if($type == 'all' || $type == '') {
            $menu[0]                = get_menu( 'tbl_user_akses', 'tbl_menu', user('id_group') );
            foreach( $menu[0] as $k_menu1 => $m ){
                // -- KASUS DAYU --
                if(isset($menu_exclusive[$m->target]) && !in_array(user('_key'), $usr_menu[$m->target])) {
                    unset($menu[0][$k_menu1]);
                }
                // -- END KASUS DAYU
                $menu[$m->id]       = get_menu( 'tbl_user_akses', 'tbl_menu', user('id_group') , $m->id );
                foreach($menu[$m->id] as $k_menu2 => $s) {
                    // -- KASUS DAYU
                    if(isset($menu_exclusive[$s->target]) && !in_array(user('_key'), $usr_menu[$s->target])) {
                        unset($menu[$m->id][$k_menu2]);
                    }
                    // -- END KASUS DAYU
                    $menu[$s->id]   = get_menu( 'tbl_user_akses', 'tbl_menu', user('id_group') , $s->id );
                    foreach($menu[$s->id] as $k_menu3 => $e) {
                        // -- KASUS DAYU
                        if(isset($menu_exclusive[$e->target]) && !in_array(user('_key'), $usr_menu[$e->target])) {
                            unset($menu[$s->id][$k_menu3]);
                        }
                        // -- END KASUS DAYU
                        $menu[$e->id]   = get_menu( 'tbl_user_akses', 'tbl_menu', user('id_group') , $e->id );
                        foreach($menu[$e->id] as $k_menu4 => $f) {
                            // -- KASUS DAYU
                            if(isset($menu_exclusive[$f->target]) && !in_array(user('_key'), $usr_menu[$f->target])) {
                                unset($menu[$e->id][$k_menu4]);
                            }
                            // -- END KASUS DAYU
                        }
                    }
...
function notifikasi() {
    $CI                 = get_instance();
    $last_count         = $CI->session->userdata('notif_count');
    ...
    $data['toast']      = $data['count'] == $last_count ? 'false' : 'true';
    $CI->session->set_userdata('notif_count',$data['count']);
            ...
            foreach($field as $k => $v) {
                if(!isset($data[$k])) {
                    ...
                    if($v == 'date') {
                        ...
                        // }
                        if($data[$k]) {
                            if(strpos($data[$k],',') !== false && strpos($data[$k],'.') !== false && ctype_digit(str_replace('-','',str_replace(['.',','],'',$data[$k])))) {
                                $c_exp = explode(',', $data[$k]);
                                if(count($c_exp) == 2) {
                                    $data[$k] = str_replace(['.',','],['','.'],$data[$k]);
                                }
                            } elseif(strpos($data[$k],'.') !== false && ctype_digit(str_replace('-','',str_replace('.','',$data[$k])))) {
                                $c_exp = explode('.', $data[$k]);
                                if(strlen($c_exp[count($c_exp)-1]) <= 3) {
                                    $data[$k] = str_replace('.','',$data[$k]);
                                }
                            } elseif(strpos($data[$k],',') !== false && ctype_digit(str_replace(',','',$data[$k]))) {
                                $c_exp = explode(',', $data[$k]);
                                if(count($c_exp) == 2) {
                                    $data[$k] = str_replace(',','.',$data[$k]);
                                }
...
function destroy_data($tabel='',$field='',$id='',$child=array(),$del_file='',$as_access='') {
    ...
    if( $menu['access_delete'] ) {
        ...
        if(is_array($field)) {
            ...
            $mandatory_role     = [1];
            $mr                 = json_decode(setting('mandatory_role'),true);
            if($mr != null && is_array($mr)) {
                foreach($mr as $_mr) {
                    $mandatory_role[]   = $_mr;
                }
            }
            if(is_array($id)) {
                $jml_del = 0;
                foreach($id as $j) {
                    if( ($tabel == 'tbl_user' && $j == 1) || ($tabel == 'tbl_user_group' && in_array($j,$mandatory_role)) ) {
                        $message = lang('izin_ditolak');
                        update_data('tbl_user_log',['respon'=>403],'id',setting('last_id_log'));
                ...
                if( ($tabel == 'tbl_user' && $id == 1) || ($tabel == 'tbl_user_group' && in_array($id, $mandatory_role)) ) {
                    $message = lang('izin_ditolak');
                    update_data('tbl_user_log',['respon'=>403],'id',setting('last_id_log'));
...
function data_serverside($config=array()) {
    ...
    $filter     = post('filter');
    $f_val      = post('f_val');
    $table      = base64_decode(post('table'));
    $limit      = post('limit');
    $offset     = post('offset');
    $sort_by    = base64_decode(post('order_by'));
    ...
    if(isset($get_arr_cache[$uri_string]) && is_array($get_arr_cache[$uri_string]) && ENVIRONMENT == 'production') {
        ...
        foreach($field as $k => $f_encode) {
            $f = base64_decode($f_encode);
            $ff = explode(' ', $f);
            ...
            $select[] = $f.' AS '.base64_decode($alias[$k]);
        }
    ...
    if($filter && is_array($filter)) {
        foreach($filter as $k => $f_encode) {
            $f = base64_decode($f_encode);
            if(trim(base64_decode($f_val[$k]),' ') != '') {
                $like[$f] = base64_decode($f_val[$k]);
    ...
    $arr_a = array('select'=>'COUNT('.base64_decode($field[0]).') AS jml','join');
    $arr_a['join'] = $join_q;
    ...
    if($is_filter) {
        ...
        $arr_q['select']    = 'COUNT('.base64_decode($field[0]).') AS jml';
...
function custom_format($value,$absolute=false) {
    if($value) {
        if($absolute) $value = abs($value);
        return $value < 0 ? '<span class="currency-negative">('.number_format(abs($value),0,',','.').')</span>' : number_format($value,0,',','.');
    } else {
        return $value == 0 ? 0 : '';
    }
}
...
function _implode($_glue='',$_arr='') {
	...
	if(is_array($arr)) {
        ...
        return trim($res,$glue);
	} else return $arr;
}

function send_wa($token='',$hp='',$message='') {
    // $token = '0b0cfbb697c2a1c81a390b7d81759eb69f6015e001720d35';
    if($token && $hp && $message) {
        $data = array(
            'phone_no' => $hp, 
            'key' => $token, 
