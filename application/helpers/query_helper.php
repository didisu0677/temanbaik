...
function get_data($table="",$attr=array(),$column=''){
    ...
    if(is_array($attr)) {
        ...
        if(isset($attr['sort_by'])) {
            if(is_array($attr['sort_by'])) {
                foreach($attr['sort_by'] as $k_sa => $sa){
                    if(strtolower($sa) == 'asc' || strtolower($sa) == 'desc' || strtolower($sa) == 'random') {
        ...
        if(isset($attr['order_by'])) {
            if(is_array($attr['order_by'])) {
                foreach($attr['order_by'] as $k_sa => $sa){
                    if(strtolower($sa) == 'asc' || strtolower($sa) == 'desc' || strtolower($sa) == 'random') {
                        $db_group->order_by($k_sa,$sa);
                    } else {
                        $db_group->order_by($sa,$sort);
...
function row_content($field='',$table='',$where='',$column='') {
